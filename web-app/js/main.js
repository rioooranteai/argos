...
async function speakMessage(text, btnEl) {
  // Toggle: Jika sedang memutar audio ini, matikan
...
async function transcribeAudioServer(audioBlob) {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/voice/stt?language=id&with_timestamps=false`, {
      method: "POST",
      body: formData,
    });
...
// Patch sendMessage to also update history
const _origSendMessage = sendMessage;
window.sendMessage = async function() {
  const question = chatInput.value.trim();
  if (!question) return;

  // call original — but we intercept the result via MutationObserver below
  await _origSendMessage();
};

// Observe chatBox for new bot messages to record in history
const _historyObserver = new MutationObserver((mutations) => {
  mutations.forEach(m => {
    m.addedNodes.forEach(node => {
      if (node.classList && node.classList.contains('bot-wrapper')) {
        const bubbles = node.querySelectorAll('.message');
        ...
        if (botBubble) {
          ...
          for (let i = idx - 1; i >= 0; i--) {
            ...
            if (userMsg) { userText = userMsg.textContent.trim(); break; }
          }
          const botText = botBubble.textContent.trim().slice(0, 60) + (botBubble.textContent.length > 60 ? '…' : '');
